...
for j in range(nbags):
    ...
    model = nn_model()
    csv_logger = CSVLogger('log'+str(j)+'.txt')
    checkpointer = ModelCheckpoint(filepath='models/Model'+str(j)+'.hdf5', verbose=1, save_best_only=True)
    fit = model.fit_generator(generator = batch_generator(xtrain, y,128, True),
        		      nb_epoch = nepochs,
        		      callbacks = [csv_logger,checkpointer],
        	              samples_per_epoch = xtrain.shape[0],
        		      verbose = 1)
    model.save('models/Last_Iter'+str(j)+'.hdf5')
    train_pred = np.exp(model.predict_generator(generator=batch_generatorp(cptrain,800,False), val_samples = xtrain.shape[0])[:,0])-200
    print '*********************Training MAE over bag ',j,' is : ',mean_absolute_error(np.exp(cpy)-200,train_pred),'    ************************'
    pred_oob += train_pred
    pred_test += np.exp(model.predict_generator(generator=batch_generatorp(xtest,800,False), val_samples = xtest.shape[0])[:,0]) - 200    
    print 'Bag ',j,' took ',time.time()-start_time,' seconds'
    
pred_oob = pred_oob*1.0/(nbags)
print 'Total MAE: ', mean_absolute_error(pred_oob,np.exp(cpy)-200)
