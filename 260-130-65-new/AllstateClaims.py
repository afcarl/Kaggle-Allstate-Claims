...
def nn_model():
    model = Sequential()
    model.add(Dense(260,W_regularizer = l1l2(),input_dim=X_train.shape[1],init='he_normal'))
    model.add(PReLU())
    model.add(Dropout(0.4))
    model.add(Dense(130,W_regularizer = l1l2(),init='he_normal'))
    model.add(PReLU())
    model.add(Dropout(0.2))
    model.add(Dense(65,W_regularizer = l1l2(),init='he_normal'))
    model.add(LeakyReLU())
    model.add(Dropout(0.2))
...
for (inTr, inTe) in folds:
    ...
    for j in range(nbags):
        ...
        earlyStopping=EarlyStopping(monitor='val_loss', patience=100, verbose=2, mode='min')
