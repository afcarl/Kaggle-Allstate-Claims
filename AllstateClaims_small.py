...
def nn_model():
    model = Sequential()
    model.add(Dense(400,W_regularizer = l1l2(),input_dim=X_train.shape[1],init='he_normal'))
    model.add(PReLU())
    model.add(Dropout(0.3))
    model.add(Dense(200,W_regularizer = l1l2(),init='he_normal'))
    model.add(PReLU())
...
for (inTr, inTe) in folds:
	...
	checkpointer = ModelCheckpoint(filepath="models/NewBest10CVModel"+str(i)+"-"+str(j)+".hdf5", verbose=1, save_best_only=True)
        fit = model.fit_generator(generator=batch_generator(X,Y,64,True),
