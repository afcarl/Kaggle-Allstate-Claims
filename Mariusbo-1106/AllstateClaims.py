...
if __name__ == "__main__":
    ...
    for i, (train_index, test_index) in enumerate(kf):
        print('\n Fold %d' % (i+1)),
        d_train, X_val = train_x.iloc[train_index], train_x.iloc[test_index]
	...
	print 'Done Training ',
        ...
        temp = np.exp(clf.predict(d_valid, ntree_limit=clf.best_ntree_limit)) - shift
        pred_oob[test_index] += temp
    ...
    pred_oob = pd.DataFrame(pred_oob,columns=['loss'])
    pred_oob['id'] = id_train
    pred_oob = pred_oob.set_index('id')
    pred_oob.to_csv('output/pred_oob'+ '5fold-average-xgb_fairobj_' + str(score) + '_' +  str(now.strftime("%Y-%m-%d-%H-%M"))+'.csv', index = True, index_label='id')
    print("train average prediction: ", mean_absolute_error(pred_oob['loss'],np.exp(train_y)-shift))
